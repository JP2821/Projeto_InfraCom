...
def checksum(porta_origem, porta_destino, comprimento):
    primera_soma = bin(porta_origem + porta_destino)[2:].zfill(
        16)  # uso a função zfill para preencher com 0 a esquerda qual quer espaço que ficar sobrando
...
def cria_pacote_cliente(porta_origem, porta_destino, comprimento, soma, seq, dados):
    pacote = bin(porta_origem)[2:].zfill(16) + bin(porta_destino)[2:].zfill(16) + bin(comprimento)[2:].zfill(16) + bin(
        soma)[2:].zfill(16) + bin(seq)[2:].zfill(1) + bin(dados)[2:].zfill(32)
    return pacote
...
def cria_pacote_servidor(porta_origem, porta_destino, comprimento, ack, seq):
    ...
    pacote = bin(porta_origem)[2:].zfill(16) + bin(porta_destino)[2:].zfill(16) + bin(comprimento)[2:].zfill(16) + bin(
        ack)[2:].zfill(1) + bin(seq)[2:].zfill(1) + bin(soma)[2:].zfill(16)
